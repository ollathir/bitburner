import {timeStamp} from "functions.js";
import {tableToHTMLString, printBox, printBox2} from "printTable.js";

var log=[];
var passInstruction = "";
var myBox = null;


/** @param {NS} ns **/
export async function main(ns) {
	ns.disableLog("disableLog"); ns.disableLog("sleep");
	//	ns.tail();
	ns.clearLog();



	if (!ns.getPlayer().hasCorporation) {
		ns.tprint( "Only use this script once you have an established corp.");
		return;
	}
	var corp = ns.corporation.getCorporation();
	if (corp.divisions.length < 2) {
		// initial Company setup
		ns.corporation.expandIndustry("Tobacco", "SmartVape");
		corp = ns.corporation.getCorporation();
		await initialCorpUpgrade(ns);
		await initCities(ns, corp.divisions[1]);

		// need to expand to cities and buy initial warehouse...
	}

	passInstruction="run";
	while (passInstruction!="kill") {
		corp = ns.corporation.getCorporation();

		display(ns);
		displayBox(ns);

		if (corp.funds > 1000000000000 && corp.divisions.length <3) {
			// with gained money, expand to the most profitable division
			ns.corporation.expandIndustry("Healthcare", "Darkcare");
			await initCities(ns, ns.corporation.getCorporation().divisions[2]);
		}

		// I don't want to be spending money on this until we have employees and warehouses in each city...
		if ( expandedToAllAreas(ns) ) {
			upgradeWilsonAdvert( ns );
		}

		for (const division of corp.divisions.reverse()) {
			if (division.type!="Tobacco" && division.type!="Healthcare") { continue; } // Only upgrade Tobacco and healthcare
			if ( !expandedToAllAreas(ns) ) {
				await initCities( ns, division );
				continue;
			} 

			if (ns.corporation.hasUnlockUpgrade("Warehouse API")) {
				upgradeWarehouses(ns, division); }
			if ( expandedToAllAreas(ns) ) {				
				upgradeCorp(ns);
			}
			await hireEmployees(ns, division);
			if (division.type=="Tobacco") {
				newProduct(ns, division); 
				doResearch(ns, division); 
			}
			if (division.type=="Healthcare") {
				newProduct(ns, division); 
				doResearch(ns, division); 
			}
			await assignEmployees2( ns, division );
		}
		if (corp.divisions.length < 3 && corp.numShares == corp.totalShares) {
			if (corp.divisions[1].products.length > 2) { // products division, divisions[0] will be agriculture
				await trickInvest(ns, corp.divisions[1]);
			}
		}

		await ns.sleep(200);
	}
	ns.tail();
	myBox.remove();
}

function expandedToAllAreas(ns) {
	var expanded = true;
	for (var division of ns.corporation.getCorporation().divisions) {
		// are we active with warehouses in all areas?
		for (var city of cities) {
			if (!expanded) { continue; }
			if ( division.cities.length != cities.length ) {
				expanded = false;
				continue;
			}
			if ( !ns.corporation.hasWarehouse( division.name, city ) ) {
				expanded = false;
				continue;
			}
			if ( ns.corporation.getOffice(division.name, city).employees.length < 3 ) {
				expanded = false;
				continue;
			}
		}
	}
	return expanded;
}

function display(ns) {
	var corp = ns.corporation.getCorporation();
	// Corp info
	ns.clearLog();
	ns.print( `Funds ${ns.nFormat(corp.funds,"0.000a")} Divisions: ${corp.divisions.length}`);
	for (const division of corp.divisions.reverse()) {
		ns.print( `${division.name} ${ns.nFormat(division.lastCycleRevenue-division.lastCycleExpenses,"0.000a")}`);
	}

	ns.print( "" );
	for (var division of corp.divisions) {
		for (var product of division.products) {
			if (ns.corporation.getProduct(division.name, product).developmentProgress < 100) {
				ns.print(division.name + " Product development progress: " + ns.corporation.getProduct(division.name, product).developmentProgress.toFixed(1) + "%");
			}
		}
	}

	ns.print( "" );
	// print log
	for (var i=(log.length-1) ; i > (log.length-20) ; i--) {
		if (i<0) { continue; }
		ns.print(log[i]);	
	}

}

function displayBox(ns) {
	var corp = ns.corporation.getCorporation();
	var myData = '';
	var table = [ ["Corporation Info", expandedToAllAreas(ns), corp.state+("_".repeat( 20-corp.state.length) )  ]];
	table.push( ["Funds", ns.nFormat(corp.funds,"0.000a")] );
	table.push( ["Profit", ns.nFormat(corp.revenue-corp.expenses,"0.000a")]);
	table.push( ["Shares", ns.nFormat(corp.numShares,"0.000a"), ns.nFormat(corp.totalShares,"0.000a")]);
	myData += tableToHTMLString(table);

	table = [];
	table.push( ["Division", "Profit", "Awareness", "Popularity", "Research", "Product 1", "Product 2", "Product 3", "Product 4", "Product 5"]);
	for (var division of corp.divisions) {
		var tableRow = [];
		tableRow.push( division.name );
		tableRow.push( ns.nFormat(division.lastCycleRevenue-division.lastCycleExpenses,"0.000a") );
		tableRow.push( ns.nFormat(division.awareness,"0.000a") );
		tableRow.push( ns.nFormat(division.popularity,"0.000a") );
		tableRow.push( ns.nFormat(division.research,"0.000a"));
		for (var product of division.products) {
			if (ns.corporation.getProduct(division.name, product).developmentProgress < 100) {
				tableRow.push(ns.corporation.getProduct(division.name, product).developmentProgress.toFixed(1) + "%");
			} else {
				tableRow.push(ns.nFormat(ns.corporation.getProduct(division.name, product).rat,"0.000a"));
			}
		}
		table.push( tableRow );
	}
	myData += tableToHTMLString(table);

	table = [];
	table.push( ["Log"] );
	// print log
	for (var i=(log.length-20) ; i < (log.length) ; i++) {
		if (i<0) { 
			table.push([""]); 
			continue; 
		}
		table.push([log[i]]);	
	}
	myData += tableToHTMLString(table);

	myBox = printBox2( corp.name, myData );
  	if (!myBox.head.getAttribute("data-listener")) { myBox.head.querySelector(".kill").addEventListener('click', () => { passInstruction="kill" } ); }
    myBox.head.setAttribute( 'data-listener', "true" ); 
}

function upgradeWilsonAdvert( ns ) {
	var advert = 0;
	var corp = ns.corporation.getCorporation();

	var cost = 0;

	// only run once tobacco and healthcare have Market-TA.II
	var hasMarketTA = true;
	for (var division of corp.divisions) {
		if (division.type=="Tobacco" || division.type=="Healthcare") {
			if (!ns.corporation.hasResearched(division.name, "Market-TA.II")) {
				hasMarketTA = false;
			}
		}
	}
	if (!hasMarketTA) { return; }
	
	while ( ns.corporation.getCorporation().funds > ns.corporation.getUpgradeLevelCost("Wilson Analytics") ) {
		ns.corporation.levelUpgrade("Wilson Analytics");	
		log.push( timeStamp() + "Upgrading Wilson Analytics to " + ns.corporation.getUpgradeLevel("Wilson Analytics") );
	}

	for (const division of corp.divisions.reverse()) {
		if (division.type!="Tobacco" && division.type!="Healthcare") continue; // Only upgrade Tobacco
		while (ns.corporation.getCorporation().funds > ns.corporation.getHireAdVertCost(division.name) ) {
			ns.corporation.hireAdVert( division.name );
			log.push( timeStamp() + "Hiring AdVert for " + division.name + " to level " + ns.corporation.getHireAdVertCount( division.name ) );
		}
	}
}

/*async function assignEmployees(ns, division, productCity ) {
		// set jobs after hiring people just in case we hire lots of people at once and setting jobs is slow
		return; // slow, using assignEmployees2
		for (const city of cities) {
			var employees = ns.corporation.getOffice(division.name, city).employees.length;
			if (ns.corporation.hasResearched(division.name, "Market-TA.II")) {
				// TODO: Simplify here. ProductCity config can always be used
				if (city == productCity) {
					await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", Math.ceil(employees / 5));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Engineer", Math.ceil(employees / 5));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Business", Math.ceil(employees / 5));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Management", Math.ceil(employees / 10));
					var remainingEmployees = employees - (3 * Math.ceil(employees / 5) + Math.ceil(employees / 10));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Training", Math.ceil(remainingEmployees));
				}
				else {
					await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", Math.floor(employees / 10));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Engineer", 1);
					await ns.corporation.setAutoJobAssignment(division.name, city, "Business", Math.floor(employees / 5));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Management", Math.ceil(employees / 100));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Research & Development", Math.ceil(employees / 2));
					var remainingEmployees = employees - (Math.floor(employees / 5) + Math.floor(employees / 10) + 1 + Math.ceil(employees / 100) + Math.ceil(employees / 2));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Training", Math.floor(remainingEmployees));
				}
			}
			else {
				if (city == productCity) {
					await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", Math.floor((employees - 2) / 2));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Engineer", Math.ceil((employees - 2) / 2));
					await ns.corporation.setAutoJobAssignment(division.name, city, "Management", 2);
				}
				else {
					await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", 1);
					await ns.corporation.setAutoJobAssignment(division.name, city, "Engineer", 1);
					await ns.corporation.setAutoJobAssignment(division.name, city, "Research & Development", (employees - 2));
				}
			}
		}
}*/

function checkWarehouses(ns, division) {
	// check that there is a warehouse in each city, if not then buy. 
	var allCities = true;
	for (var city of cities) {
		if ( !ns.corporation.hasWarehouse( division.name, city )) {
			ns.corporation.purchaseWarehouse( division.name, city );
			log.push( timeStamp() + "Opening warehouse in " + city + " for " + division.name );
			allCities = false;
		} 
	}
	return allCities;
}

async function hireEmployees(ns, division, productCity = "Aevum") {
	if (!ns.corporation.hasUnlockUpgrade("Office API")) {
		return;
	}
	if (!checkWarehouses(ns, division)) { 
		return; 
	}

	for (var city of cities) {
		if ( ns.corporation.getOffice(division.name, city).employees.length < ns.corporation.getOffice(division.name, city).size ) {
			await ns.corporation.hireEmployee(division.name, city);
			log.push( timeStamp() + " Hiring employee at " + city + " for " + division.name );
		}
	}

	var employees = ns.corporation.getOffice(division.name, productCity).employees.length;
	while (ns.corporation.getCorporation().funds > (cities.length * ns.corporation.getOfficeSizeUpgradeCost(division.name, productCity, 3))) {
		// upgrade all cities + 3 employees if sufficient funds
		log.push(timeStamp() + division.name + " Upgrade office size");
		for (const city of cities) {
			ns.corporation.upgradeOfficeSize(division.name, city, 3);
			for (var i = 0; i < 3; i++) {
				await ns.corporation.hireEmployee(division.name, city);
			}
		}
	}
	/*	if (ns.corporation.getOffice(division.name, productCity).employees.length > employees) {
		await assignEmployees(ns, division, productCity);
	}*/
}

function upgradeWarehouses(ns, division) {
	if (!checkWarehouses( ns, division )) {
		return;
	}
	for (const city of division.cities) {
		// check if warehouses are near max capacity and upgrade if needed
		var cityWarehouse = ns.corporation.getWarehouse(division.name, city);
		if (cityWarehouse.sizeUsed > 0.9 * cityWarehouse.size) {
			if (ns.corporation.getCorporation().funds > ns.corporation.getUpgradeWarehouseCost(division.name, city)) {
				log.push(timeStamp() + division.name + " Upgrade warehouse in " + city);
				ns.corporation.upgradeWarehouse(division.name, city);
			}
		}
	}
	/*	if (ns.corporation.getUpgradeLevel("Wilson Analytics") > 20) {
		// Upgrade AdVert.Inc after a certain amount of Wilson Analytivs upgrades are available
		if (ns.corporation.getCorporation().funds > (4 * ns.corporation.getHireAdVertCost(division.name))) {
			log.push(timeStamp() + division.name + " Hire AdVert");
			ns.corporation.hireAdVert(division.name);
		}
	}*/
}

function upgradeCorp(ns) {
	if (!ns.corporation.hasUnlockUpgrade("Warehouse API") && ns.corporation.getUnlockUpgradeCost("Warehouse API") < ns.corporation.getCorporation().funds ) { ns.corporation.unlockUpgrade("Warehouse API"); }
	if (!ns.corporation.hasUnlockUpgrade("Office API") && ns.corporation.getUnlockUpgradeCost("Office API") < ns.corporation.getCorporation().funds ) { ns.corporation.unlockUpgrade("Office API"); }

	for (const upgrade of upgradeList) {
		// purchase upgrades based on available funds and priority; see upgradeList
		if (ns.corporation.getCorporation().funds > (upgrade.prio * ns.corporation.getUpgradeLevelCost(upgrade.name))) {
			// those two upgrades ony make sense later once we can afford a bunch of them and already have some base marketing from DreamSense
			if ((upgrade.name != "ABC SalesBots" && upgrade.name != "Wilson Analytics") || (ns.corporation.getUpgradeLevel("DreamSense") > 20)) {
				log.push(timeStamp() + "Upgrade " + upgrade.name + " to " + (ns.corporation.getUpgradeLevel(upgrade.name) + 1));
				ns.corporation.levelUpgrade(upgrade.name);
			}
		}
	}
	if (!ns.corporation.hasUnlockUpgrade("Shady Accounting") && ns.corporation.getUnlockUpgradeCost("Shady Accounting") * 2 < ns.corporation.getCorporation().funds) {
		log.push(timeStamp() + "Unlock Shady Accounting");
		ns.corporation.unlockUpgrade("Shady Accounting");
	}
	else if (!ns.corporation.hasUnlockUpgrade("Government Partnership") && ns.corporation.getUnlockUpgradeCost("Government Partnership") * 2 < ns.corporation.getCorporation().funds) {
		log.push(timeStamp() + "Unlock Government Partnership");
		ns.corporation.unlockUpgrade("Government Partnership");
	}
}

async function trickInvest(ns, division, productCity = "Aevum") {
	log.push(timeStamp() + "Prepare to trick investors");
	for (var product of division.products) {
		// stop selling products
		ns.corporation.sellProduct(division.name, productCity, product, "0", "MP", true);
	}

	for (const city of cities) {
		// put all employees into production to produce as fast as possible 
		const employees = ns.corporation.getOffice(division.name, city).employees.length;

		await ns.corporation.setAutoJobAssignment(division.name, city, "Engineer", 0);
		await ns.corporation.setAutoJobAssignment(division.name, city, "Management", 0);
		await ns.corporation.setAutoJobAssignment(division.name, city, "Research & Development", 0);
		await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", employees - 2); // workaround for bug
		await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", employees - 1); // workaround for bug
		await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", employees);
	}

	log.push(timeStamp() + "Wait for warehouses to fill up");
	let allWarehousesFull = false;
	while (!allWarehousesFull) {
		allWarehousesFull = true;
		for (const city of cities) {
			if (ns.corporation.getWarehouse(division.name, city).sizeUsed <= (0.98 * ns.corporation.getWarehouse(division.name, city).size)) {
				allWarehousesFull = false;
				break;
			}
		}
		await ns.sleep(5000);
	}
	log.push(timeStamp() + "Warehouses are full, start selling");

	var initialInvestFunds = ns.corporation.getInvestmentOffer().funds;
	log.push(timeStamp() + "Initial investmant offer: " + ns.nFormat(initialInvestFunds, "0.0a"));
	for (const city of cities) {
		// put all employees into business to sell as much as possible 
		const employees = ns.corporation.getOffice(division.name, city).employees.length;
		await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", 0);
		await ns.corporation.setAutoJobAssignment(division.name, city, "Business", employees - 2); // workaround for bug
		await ns.corporation.setAutoJobAssignment(division.name, city, "Business", employees - 1); // workaround for bug
		await ns.corporation.setAutoJobAssignment(division.name, city, "Business", employees);
	}
	for (var product of division.products) {
		// sell products again
		ns.corporation.sellProduct(division.name, productCity, product, "MAX", "MP", true);
	}

	while (ns.corporation.getInvestmentOffer().funds < (4 * initialInvestFunds)) {
		// wait until the stored products are sold, which should lead to huge investment offers
		await ns.sleep(200);
	}

	log.push(timeStamp() + "Investment offer for 10% shares: " + ns.nFormat(ns.corporation.getInvestmentOffer().funds, "0.0a"));
	log.push(timeStamp() + "Funds before public: " + ns.nFormat(ns.corporation.getCorporation().funds, "0.0a"));

	ns.corporation.goPublic(800e6);

	log.push(timeStamp() + "Funds after  public: " + ns.nFormat(ns.corporation.getCorporation().funds, "0.0a"));

	for (const city of cities) {
		// set employees back to normal operation
		const employees = ns.corporation.getOffice(division.name, city).employees.length;
		await ns.corporation.setAutoJobAssignment(division.name, city, "Business", 0);
		if (city == productCity) {
			await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", 1);
			await ns.corporation.setAutoJobAssignment(division.name, city, "Engineer", (employees - 2));
			await ns.corporation.setAutoJobAssignment(division.name, city, "Management", 1);
		}
		else {
			await ns.corporation.setAutoJobAssignment(division.name, city, "Operations", 1);
			await ns.corporation.setAutoJobAssignment(division.name, city, "Research & Development", (employees - 1));
		}
	}

	// with gained money, expand to the most profitable division
	ns.corporation.expandIndustry("Healthcare", "Healthcare");
	await initCities(ns, ns.corporation.getCorporation().divisions[1]);
}

function doResearch(ns, division) {
	const laboratory = "Hi-Tech R&D Laboratory"
	const marketTAI = "Market-TA.I";
	const marketTAII = "Market-TA.II";

	if (!ns.corporation.hasUnlockUpgrade("Office API")) return; // need office API

	if (!ns.corporation.hasResearched(division.name, laboratory)) {
		// always research labaratory first
		if (division.research > ns.corporation.getResearchCost(division.name, laboratory)) {
			log.push(timeStamp() + division.name + " Research " + laboratory);
			ns.corporation.research(division.name, laboratory);
		}
	}
	else if (!ns.corporation.hasResearched(division.name, marketTAII)) {
		// always research Market-TA.I plus .II first and in one step
		var researchCost = ns.corporation.getResearchCost(division.name, marketTAI)
			+ ns.corporation.getResearchCost(division.name, marketTAII);

		if (division.research > researchCost * 1.1) {
			log.push(timeStamp() + division.name + " Research " + marketTAI);
			ns.corporation.research(division.name, marketTAI);
			log.push(timeStamp() + division.name + " Research " + marketTAII);
			ns.corporation.research(division.name, marketTAII);
			for (var product of division.products) {
				ns.corporation.setProductMarketTA1(division.name, product, true);
				ns.corporation.setProductMarketTA2(division.name, product, true);
			}
		}
		return;
	}
	else {
		for (const researchObject of researchList) {
			// research other upgrades based on available funds and priority; see researchList
			if (!ns.corporation.hasResearched(division.name, researchObject.name)) {
				if (division.research > (researchObject.prio * ns.corporation.getResearchCost(division.name, researchObject.name))) {
					log.push(timeStamp() + division.name + " Research " + researchObject.name);
					ns.corporation.research(division.name, researchObject.name);
				}
			}
		}
	}
}

function newProduct(ns, division) {
	//ns.print("Products: " + division.products);
	var productNumbers = [];
	for (var product of division.products) {
		if (ns.corporation.getProduct(division.name, product).developmentProgress < 100) {
		//	log.push(division.name + " Product development progress: " + ns.corporation.getProduct(division.name, product).developmentProgress.toFixed(1) + "%");
			return false;
		}
		else {
			productNumbers.push(product.charAt(product.length - 1));
			// initial sell value if nothing is defined yet is 0
			if (ns.corporation.getProduct(division.name, product).sCost == 0) {
				log.push(timeStamp() + division.name + " Start selling product " + product);				
				ns.corporation.sellProduct(division.name, "Sector-12", product, "MAX", "MP", true);
				if (ns.corporation.hasUnlockUpgrade("Office API")) { //need office API
					if (ns.corporation.hasResearched(division.name, "Market-TA.II")) {	
						ns.corporation.setProductMarketTA1(division.name, product, true);
						ns.corporation.setProductMarketTA2(division.name, product, true);
					}
				}
			}
		}
	}

	var numProducts = 3;
	if (ns.corporation.hasUnlockUpgrade("Office API")) { //need office API
		// amount of products which can be sold in parallel is 3; can be upgraded
		if (ns.corporation.hasResearched(division.name, "uPgrade: Capacity.I")) {
			numProducts++;
			if (ns.corporation.hasResearched(division.name, "uPgrade: Capacity.II")) {
				numProducts++;
			}
		}
	}

	if (productNumbers.length >= numProducts) {
		// discontinue the oldest product if over max amount of products
		log.push(timeStamp() + division.name + " Discontinue product " + division.products[0]);
		ns.corporation.discontinueProduct(division.name, division.products[0]);
	}

	// get the product number of the latest product and increase it by 1 for the mext product. Product names must be unique. 
	var newProductNumber = 0;
	if (productNumbers.length > 0) {
		newProductNumber = parseInt(productNumbers[productNumbers.length - 1]) + 1;
		// cap product numbers to one digit and restart at 0 if > 9.
		if (newProductNumber > 9) {
			newProductNumber = 0;
		}
	}
	var newProductName="Product-";
	if (division.type=="Tobacco") {
		var newProductName = "MultiVape-" + newProductNumber;
	} else if (division.type=="Healthcare") {
		var newProductName = "Darkcare-" + newProductNumber;
	} else {
		var newProductName = "Product-" + newProductNumber;
	}

	var productInvest = 1e9;
	if (ns.corporation.getCorporation().funds < (2 * productInvest)) {
		if (ns.corporation.getCorporation().funds <= 0) {
			log.push(timeStamp() + "WARN negative funds, cannot start new product development " + ns.nFormat(ns.corporation.getCorporation().funds, "0.0a"));
			return;
			// productInvest = 0; // product development with 0 funds not possible if corp has negative funds
		}
		else {
			productInvest = Math.floor(ns.corporation.getCorporation().funds / 2);
		}
	}
	log.push(timeStamp() + "Start new product development " + newProductName);
	ns.corporation.makeProduct(division.name, "Aevum", newProductName, productInvest, productInvest);
}

async function initCities(ns, division, productCity = "Aevum") {
	for (const city of cities) {
		log.push("Expand " + division.name + " to City " + city);
		if (!division.cities.includes(city)) {
			ns.corporation.expandCity(division.name, city);
			ns.corporation.purchaseWarehouse(division.name, city);
		}

		//ns.corporation.setSmartSupply(division.name, city, true); // does not work anymore, bug?

		if (city != productCity) {
			if ( ns.corporation.getOffice(division.name, city).employees.length < 3 ) {
				// setup employees
				for (let i = 0; i < 3; i++) {
					await ns.corporation.hireEmployee(division.name, city);
				}
				await ns.corporation.setAutoJobAssignment(division.name, city, "Research & Development", 3);
			}
			const warehouseUpgrades = 3;
			if ( ns.corporation.getWarehouse( division.name, city ).size < 300 ) {
				ns.corporation.upgradeWarehouse(division.name, city);
			}
		}
		else {
			if ( !ns.corporation.hasWarehouse( division.name, city ) ) {
				ns.corporation.purchaseWarehouse( division.name, city );
			} else {
				const warehouseUpgrades = 3;
				// get a bigger warehouse in the product city. we can produce and sell more here
				if ( ns.corporation.getWarehouse(division.name, city).size < 300 ) {
					ns.corporation.upgradeWarehouse(division.name, city);
				}
				// get more employees in the main product development city
				const newEmployees = 9;
				if ( ns.corporation.getOffice( division.name, city ).size < newEmployees ) {
					ns.corporation.upgradeOfficeSize(division.name, productCity, newEmployees);	
				}
				if ( ns.corporation.getOffice( division.name, city ).employees.length < newEmployees ) {
					await ns.corporation.hireEmployee(division.name, productCity);
				}
				await ns.corporation.setAutoJobAssignment(division.name, productCity, "Operations", 4);
				await ns.corporation.setAutoJobAssignment(division.name, productCity, "Engineer", 6);
				await ns.corporation.setAutoJobAssignment(division.name, productCity, "Management", 2);
			}
		}
	}

	if (division.products.length == 0) {
		ns.corporation.makeProduct(division.name, productCity, "Product-0", "1e9", "1e9");
	}
}

async function doAssign( ns, division, city, job, number ) {
	var officeEmps = ns.corporation.getOffice( division, city ).employeeJobs;
	if ( officeEmps[job] != number ) {
		await ns.corporation.setAutoJobAssignment( division, city, job, number );
		log.push( timeStamp() + division + ": " + city + " " + job + " " + officeEmps[job] + "/" + number );
	} else {
		//log.push( division + ": " + city + " " + job + " " + officeEmps[job] + "/" + number );
	}
}

async function assignEmployees2(ns, division, productCity = "Aevum" ) {
		// set jobs after hiring people just in case we hire lots of people at once and setting jobs is slow
		for (const city of cities) {
			var employees = ns.corporation.getOffice(division.name, city).employees.length;
			if (ns.corporation.hasResearched(division.name, "Market-TA.II")) {
				// TODO: Simplify here. ProductCity config can always be used
				if (city == productCity) {
					
					await doAssign(ns, division.name, city, "Operations", Math.ceil(employees / 5));
					await doAssign(ns, division.name, city, "Engineer", Math.ceil(employees / 5));
					await doAssign(ns, division.name, city, "Business", Math.ceil(employees / 5));
					await doAssign(ns, division.name, city, "Management", Math.ceil(employees / 10));
					await doAssign(ns, division.name, city, "Research & Development", 0);
					var remainingEmployees = employees - (3 * Math.ceil(employees / 5) + Math.ceil(employees / 10));
					await doAssign(ns, division.name, city, "Training", Math.ceil(remainingEmployees));
				}
				else {
					await doAssign(ns, division.name, city, "Operations", Math.floor(employees / 10));
					await doAssign(ns, division.name, city, "Engineer", 1);
					await doAssign(ns, division.name, city, "Business", Math.floor(employees / 5));
					await doAssign(ns, division.name, city, "Management", Math.ceil(employees / 100));
					await doAssign(ns, division.name, city, "Research & Development", Math.ceil(employees / 2));
					var remainingEmployees = employees - (Math.floor(employees / 5) + Math.floor(employees / 10) + 1 + Math.ceil(employees / 100) + Math.ceil(employees / 2));
					await doAssign(ns, division.name, city, "Training", Math.floor(remainingEmployees));
				}
			}
			else {
				if (city == productCity) {
					await doAssign(ns, division.name, city, "Operations", Math.floor((employees - 2) / 2));
					await doAssign(ns, division.name, city, "Engineer", Math.ceil((employees - 2) / 2));
					await doAssign(ns, division.name, city, "Business", 0);
					await doAssign(ns, division.name, city, "Management", 2);
					await doAssign(ns, division.name, city, "Research & Development", 0);
					await doAssign(ns, division.name, city, "Training", 0);
				}
				else {
					await doAssign(ns, division.name, city, "Operations", 1);
					await doAssign(ns, division.name, city, "Engineer", 1);
					await doAssign(ns, division.name, city, "Business", 0);
					await doAssign(ns, division.name, city, "Management", 0);
					await doAssign(ns, division.name, city, "Research & Development", (employees - 2));
					await doAssign(ns, division.name, city, "Training", 0);
				}
			}
		}
}

function upgradeToLevel( ns, upgradeName, level ) {
	if (!ns.corporation.hasUnlockUpgrade(upgradeName)) {
		ns.corporation.unlockUpgrade(upgradeName);
	}
	while ( ns.corporation.getUpgradeLevel( upgradeName ) < level ) {
		ns.corporation.levelUpgrade( upgradeName );
	}
}

async function initialCorpUpgrade(ns) {
	log.push("unlock upgrades");
	upgradeToLevel(ns, "Smart Storage", 5); 
	upgradeToLevel(ns, "DreamSense", 2);
	// upgrade employee stats
	upgradeToLevel(ns,"Nuoptimal Nootropic Injector Implants",2);
	upgradeToLevel(ns,"Speech Processor Implants",2);
	upgradeToLevel(ns,"Neural Accelerators",2);
	upgradeToLevel(ns,"FocusWires",2);
}

const cities = ["Sector-12", "Aevum", "Volhaven", "Chongqing", "New Tokyo", "Ishima"];

const upgradeList = [
	// lower priority value -> upgrade faster
	{ prio: 2, name: "Project Insight", },
	{ prio: 2, name: "DreamSense" },
	{ prio: 4, name: "ABC SalesBots" },
	{ prio: 4, name: "Smart Factories" },
	{ prio: 4, name: "Smart Storage" },
	{ prio: 8, name: "Neural Accelerators" },
	{ prio: 8, name: "Nuoptimal Nootropic Injector Implants" },
	{ prio: 8, name: "FocusWires" },
	{ prio: 8, name: "Speech Processor Implants" },
	{ prio: 8, name: "Wilson Analytics" },
];

const researchList = [
	// lower priority value -> upgrade faster
	{ prio: 10, name: "Overclock" },
	{ prio: 10, name: "uPgrade: Fulcrum" },
	{ prio: 3, name: "uPgrade: Capacity.I" },
	{ prio: 4, name: "uPgrade: Capacity.II" },
	{ prio: 10, name: "Self-Correcting Assemblers" },
	{ prio: 21, name: "Drones" },
	{ prio: 4, name: "Drones - Assembly" },
	{ prio: 10, name: "Drones - Transport" },
	{ prio: 26, name: "Automatic Drug Administration" },
	{ prio: 10, name: "CPH4 Injections" },
];