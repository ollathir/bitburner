import {tableToHTMLString, printBox, printBox2,printTable,printTableToPort,convertMSToHHMMSS } from "printTable.js";
...
import {maxRam, maxTotalThreads, maxThreads} from "/hackAll_v4/functions.js";

// is there any way to brute force initial growth?

var homeReserve=500;
const hackAmt = 0.9;
const minHackChance = 0.40;
const iBackdoor=false;
const sHack = "/hackAll_v4/pureHack.js";
const sWeaken = "/hackAll_v4/pureWeaken.js";
const sGrow = "/hackAll_v4/pureGrow.js";
const sMonitor = "activity.js";
const weakenFirst=false;
const noloop=false; // should the threads be run indefinately?
const switchToFormulasAtRam = 10000; 
...
async function runScript( ns, sScript, target, threads ) { 
	...
	var openServers = list_open_servers( ns, includeHacknetServers );
	...
	for (var openServer of openServers)
	{
		...
		if (openServer=="home") { freeRam -= homeReserve; } // if home, knock off reserve
	...
	for (var target of targets) {
		...
		while ( (ns.getServerSecurityLevel(target) > (ns.getServerMinSecurityLevel(target)+1)) || (ns.getServerMoneyAvailable(target) <= (ns.getServerMaxMoney(target)*0.9)) ) { 
			...
			for (var openServer of openServers) {
				...
				if (openServer=="home") { serverFreeRam -= homeReserve; }
...
//	ns.tail();
	...
	if (!isNaN(ns.args[0])) { homeReserve=ns.args[0]; } // pass in homeReserve as arg[0]
...
//	while (true)
	{
		...
		for (var target of hackServers)
		{
			...
			if ( ns.fileExists("Formulas.exe") && maxRam(ns,homeReserve)>switchToFormulasAtRam ) {
		...
		// set up before Formulas.exe is available
				...
				if ( mtt < 200 ) { 
					...
					max = Math.ceil(maxRam( ns, homeReserve ) / 400);
				}*/
...
//					totalCashToHack += ns.getServerMaxMoney(quickScanServer);
		...
		if (ns.fileExists("Formulas.exe")) {
			await ns.writePort( port, `Memory requirements: Hack ${sHackMem} Grow ${sGrowMem} Weaken ${sWeakenMem} Highest ${sHighMem} Hacking ${ns.nFormat(hackAmt*100,"0")}% of server funds. Min hack chance ${minHackChance}` );
			myHeader += `<br>Memory requirements: Hack ${sHackMem} Grow ${sGrowMem} Weaken ${sWeakenMem} Highest ${sHighMem} Hacking ${ns.nFormat(hackAmt*100,"0")}% of server funds. Min hack chance ${minHackChance}`;
		} else {
			await ns.writePort( port, `Memory requirements: Hack ${sHackMem} Grow ${sGrowMem} Weaken ${sWeakenMem} Highest ${sHighMem}.` );
			myHeader += `<br>Memory requirements: Hack ${sHackMem} Grow ${sGrowMem} Weaken ${sWeakenMem} Highest ${sHighMem}.`;
		}
		await ns.writePort( port, `Total available Ram: ${maxRam(ns, homeReserve)} allowing ${maxTotalThreads(ns,sHighMem, homeReserve)} threads. Total threads running ${totalThreads}. (Threads Required: ${totalThreadsForAllHacks})`);
		myHeader += `<br>Total available Ram: ${maxRam(ns, homeReserve)} allowing ${maxTotalThreads(ns,sHighMem, homeReserve)} threads. Total threads running ${totalThreads}. (Threads Required: ${totalThreadsForAllHacks})`;
		await ns.writePort( port, "Income since last installed augmentations: $"+ns.nFormat(ns.getScriptIncome()[1],"0.000a") + '/sec' );
		myHeader += "<br>Income since last installed augmentations: $"+ns.nFormat(ns.getScriptIncome()[1],"0.000a") + '/sec';

		table.splice(0,0,headers);		
		var myData = "";
		myData += tableToHTMLString( table );
		myData = myHeader + myData;

		await printTableToPort( ns, table, port );
		await printTable( ns, table );

		myBox = printBox2( "hackAll_v4", myData );
	  	if (!myBox.head.getAttribute("data-listener")) { 
		    myBox.head.querySelector(".kill").addEventListener('click', () => { myBox.head.setAttribute( 'data-listener', "kill" ); } ); 
