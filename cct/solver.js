/** @param {NS} ns */ // 695414552
import {list_servers} from "getServers.js";

/*export async function main(ns) {

	var result = solverLargestPrime(ns, ns.args[0]);
	ns.tprint(result);
}*/
...
//    let listServers = ["home"];
	...
	var listServers = list_servers(ns);
...
/*
    ...
    while (true) {
      ...
      //  ns.clearLog();
        ...
        for (var server of listServers) {
            var ccts = ns.ls(server, ".cct");
            for (var cct of ccts) {
                ns.print( `Found ${cct} on ${server} type ${ns.codingcontract.getContractType(cct, server)}.`);
            }
        }
        ...
        for (let z = 0; z < listFiles.length; z++) {
            ns.print( listServers[listIndex] + "/" + listFiles[z] + ":");
            ...
            switch (inputType) {
                ...
                case "Sanitize Parentheses in Expression":
                    outputData = fixParentheses(inputData);
                    outputResult = ns.codingcontract.attempt(outputData, listFiles[z], listServers[listIndex]);
                    ns.print([listServers[listIndex],
                        listFiles[z],
                        inputType,
                ...
                default:
                    ...
                    ns.codingcontract.attempt("a", listFiles[z], listServers[listIndex]); // fail contract to clear it
                    break;
            }
        }
    }
