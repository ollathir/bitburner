...
import {printTable, convertMSToHHMMSS, tableToHTMLString, printBox2} from "printTable.js";
import {timeStamp, numberWithCommas, formatMem, hackAll} from "functions.js";

var sHack = "/hackAll_v6/hack.js";
var sGrow =  "/hackAll_v6/grow.js";
var sWeaken =  "/hackAll_v6/weaken.js";
...
//	ns.tail();
	...
	currentTick = getNextTick(ns);

	// main loop
	passInstruction = "run";
	var interval = 0;
	while (passInstruction!="kill") {
		if (interval>cycleTime) {
...
async function processQueue( ns ) {
	...
	for (var process of queue) {
		if ( !process.hasRun && curTime>=process.timeToRun ) {
			ns.print( timeStamp() + process.script + " " + process.target + " " + process.threads + " " + process.tick );
			var secDelta = ns.getServerSecurityLevel(process.target)-ns.getServerMinSecurityLevel(process.target);
			if (process.script == sHack) {
				if ( ns.getServerMoneyAvailable(process.target)>(ns.getServerMaxMoney(process.target)*0.90) && secDelta < 5 ) {
					await runScript( ns, process.script, process.target, process.threads, process.tick );
				}
			} else if (process.script == sGrow) {
				if ( secDelta < 5 ) {
					await runScript( ns, process.script, process.target, process.threads, process.tick );
				}
			} else {
	...
	while (found) {
		...
		for (var i = 0 ; i < queue.length ; i++) {
			if (found) { continue; } // not sure what happens to array indexing when item removed so removing only 1 item at a time.
			if (curTime > queue[i].finishTime) {
//			if (queue[i].hasRun ) { // has run or is more than cycleTime overdue to start
				queue.splice( i, 1 );
				found = true;
			}
		}
...
async function addHacks( ns ) {
	...
	for (var target of targets) {
		...
		var curTime = ns.getTimeSinceLastAug();
		var timeToHack = curTime + tWeaken - tHack - 2;
		var timeToGrow = curTime + tWeaken - tGrow - 1;
		...
		var process = {
			...
			timeToRun : timeToHack,
			finishTime : curTime + tWeaken,
			pid : null,
			hasRun : false,
		}
		queue.push(process);
...
export async function runScript( ns, sScript, target, threads, tick ) { 
	...
	for (var openServer of openServers)
	{
		...
		var tempPid = null;
		if ( tempPid = ns.exec( sScript, openServer, threadsToRun, target, tick )) { 
			runningThreads += threadsToRun;
			pid.push( tempPid );
		}
	}
	return;
...
function getNextTick(ns) {
	...
	for (var target of targets) {
		...
		for (var process of processes) {
			if (process.filename==sWeaken && process.args[1] > myTick && !isNaN(process.args[1])) { 
				myTick = process.args[1]+1;
			}
		}
	}
	return myTick;
...
//	await printTable( ns, table );
	...
	var myData = `Threads running: ${numberWithCommas(totalThreads)} / ${numberWithCommas(maxTotalThreads(ns,1.75))} Memory used: ${formatMem(memoryUsed)} / ${formatMem(maxRam(ns))} Current cycle: ${currentTick}`;
	myData += "<br>" + `Earnings: ${ns.nFormat(ns.getScriptIncome()[0],"0.000a")} / sec   (Instruction: ${passInstruction}) Items in queue: ${queue.length}`;
	myData += tableToHTMLString(table);
	myBox = printBox2( "hackAll_v6", myData ); 	
