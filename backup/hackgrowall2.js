...
export async function main(ns) {
    ...
    for (var server of servers) {
        ...
        if (ns.hasRootAccess(server)) { // hopefully now hacked, copy our files.
            if (!ns.fileExists(growWeaken, server)) {
    ...
    for (var server of servers) {
        ...
        if ((runThreadsHack>0 || runThreadsWeaken>0) && debug==1){
            ns.tprint( `Hacking: ${server} Free Ram: ${ns.nFormat(ns.getServerMaxRam(server)-ns.getServerUsedRam(server),"0.0")} Threads: Hack: ${runThreadsHack} Weaken: ${runThreadsWeaken}`);
        }
    }
    ns.tprint( `Hacking ${target} Successful threads: Hack: ${currentHacked}/${hackMax} Weaken: ${currentWeaken}/${weakenMax}`);
