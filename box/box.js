...
let createItem2 = (title, content, icon, ...classes) => {
  let item = sidebar.appendChild(elemFromHTML(`<div class="${classes.join(" ")}"><div class="head"><a class="icon">${icon}</a><span class=title>${title}</span><a class="icon kill">\uead7</a><a class="icon collapser">\ueab7</a><a class="icon close">\ueab8</a></div><div class="body">${content}</div></div>`));
  Object.assign(item, {
    head: item.querySelector(".head"),
    body: item.querySelector(".body"),
    toggleType: () => ["box", "sbitem"].forEach(cl => item.classList.toggle(cl)),
...
export let createBox2 = (title, content, icon = "\uea74", ...classes) => createItem2(title, content, icon, ...classes, "box");